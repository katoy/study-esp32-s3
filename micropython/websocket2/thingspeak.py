...
class ThingSpeakClient:
    ...
    def get_status_message(self):
        """現在のThingSpeakクライアントのステータスを返します。"""
        if not self.enabled:
            return '無効'
        if not self.urequests_available:
            return 'urequests未対応'
        if not self.api_key:
            return 'API Key未設定'
        ...
        if self.last_send_time > 0:
            elapsed_sec = (time.ticks_ms() - self.last_send_time) / 1000
            if elapsed_sec < 300:  # 5分以内
                return f'正常 (最終送信: {elapsed_sec:.0f}秒前)'
            else:
                return f'停止中 (最終送信: {elapsed_sec:.0f}秒前)'
        else:
            return '待機中'
