...
class SensorManager:
    def __init__(self, pin, use_internal_pullup, log_func, valid_temp_range, valid_hum_range, max_history_size):
        ...
        self.websocket_server = None  # WebSocketサーバーへの参照
        ...
        for attempt in range(max_retries):
            ...
            if self.valid_temp_range[0] <= temp <= self.valid_temp_range[1] and \
               self.valid_hum_range[0] <= hum <= self.valid_hum_range[1]:
                ...
                # WebSocketクライアントに通知
                self._notify_websocket_clients(sensor_data)
    ...
    def _notify_websocket_clients(self, sensor_data):
        """新しいセンサーデータをWebSocketクライアントに送信"""
        if self.websocket_server and self.websocket_server.get_client_count() > 0:
            try:
                import ujson as json
                # format_jst_timeを使用するため、時刻をフォーマット
                # (main.pyから関数を渡すかグローバルにアクセス)
                message = json.dumps({
                    'type': 'sensor_data',
                    'data': sensor_data
                })
                self.websocket_server.broadcast_message(message)
                self.log(f"WebSocketで送信: {sensor_data['temperature']}°C, {sensor_data['humidity']}%", "websocket", "debug")
            except Exception as e:
                self.log(f"WebSocket送信エラー: {e}", "websocket", "error")
