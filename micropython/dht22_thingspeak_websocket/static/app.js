class DHT22MonitorApp {
    constructor() {
        ...
        this.WS_RECONNECT_INTERVAL_MS = 5000;
    ...
    connectWebSocket() {
        if (this.state.websocketConnection) {
            return; // 既に接続中
        }
        ...
        try {
            ...
            this.log(`WebSocket接続を試行中: ${wsUrl}`, 'websocket', 'info');
            this.setStatus('yellow', 'WebSocket接続中...');

            this.state.websocketConnection = new WebSocket(wsUrl);

            this.state.websocketConnection.onopen = () => {
                this.log('WebSocketに接続しました', 'websocket', 'info');
                this.setStatus('green', 'WebSocket接続済み');
                this.state.reconnectAttempts = 0;

                if (this.elements.intervalText) {
                    this.elements.intervalText.textContent = 'リアルタイム更新';
                }
            ...
            this.state.websocketConnection.onmessage = (event) => {
                try {
                    ...
                    this.log(`WebSocketメッセージ解析エラー: ${e.message}`, 'websocket', 'error');
                }
            };

            this.state.websocketConnection.onclose = (event) => {
                this.log(`WebSocket接続が閉じられました (code: ${event.code})`, 'websocket', 'warning');
                this.state.websocketConnection = null;
                this.setStatus('red', 'WebSocket切断');

                // 再接続を試行
                if (this.state.reconnectAttempts < this.state.maxReconnectAttempts) {
                    this.state.reconnectAttempts++;
                    this.log(`${this.WS_RECONNECT_INTERVAL_MS / 1000}秒後にWebSocket再接続を試行 (${this.state.reconnectAttempts}/${this.state.maxReconnectAttempts})`, 'websocket', 'info');
                    setTimeout(() => this.connectWebSocket(), this.WS_RECONNECT_INTERVAL_MS);
                } else {
                    this.log('WebSocket再接続回数上限に達しました。HTTPポーリングに切り替えます', 'websocket', 'warning');
            ...
            this.state.websocketConnection.onerror = (error) => {
                this.log(`WebSocketエラーが発生しました: ${error}`, 'websocket', 'error');
                this.setStatus('red', 'WebSocketエラー');
            };

        } catch (e) {
            this.log(`WebSocket接続エラー: ${e.message}`, 'websocket', 'error');
            this.setStatus('red', 'WebSocket接続エラー');
            // フォールバックでHTTPポーリング開始
            this.state.websocketEnabled = false;
            this.startPolling();
        }
    ...
    handleWebSocketMessage(message) {
        if (message.type === 'sensor_data' && message.data) {
            ...
            // /api/realtime フォーマットに変換
            const realtimeData = {
                type: 'dht11',
                temp_c: sensorData.temperature,
                hum_pct: sensorData.humidity,
                timestamp: this.formatTimestamp(sensorData.unix_time),
                measurement_count: sensorData.measurement_count || 0
            };

            this.updateSensorReadings(realtimeData);
            this.log(`WebSocketでデータ受信: ${sensorData.temperature}°C, ${sensorData.humidity}%`, 'websocket', 'debug');
        } else {
            this.log(`未知のWebSocketメッセージタイプ: ${message.type}`, 'websocket', 'warning');
        }
    }

    formatTimestamp(unixTime) {
        // unix_timeを日本時間の文字列に変換
        if (!unixTime) return '--';
    ...
    stopPolling() {
        ...
        this.log('HTTPポーリングを停止しました', 'network', 'info');
    }
}
